...
// import { GetRelatedProductsQuery } from './related-products-query';
import { GetRelatedProductsQuery } from './cross-sale/query';
...
export function CartClient<LineItem extends CartLineItem>({
  ...
  // // Extract and console log cart product IDs as an array
  const cartProductIds = optimisticLineItems
    .filter(item => item.typename !== 'CartGiftCertificate') // Exclude gift certificates
    .map(item => {
      // Use productEntityId if available, otherwise extract from id
      return 'productEntityId' in item ? item.productEntityId : null;
    })
    .filter((id): id is number => id !== null && !isNaN(id));

  console.log('Cart Product IDs Array:', cartProductIds);

  // // Call GraphQL API with cart product IDs
  const fetchRelatedProducts = async () => {
    if (cartProductIds.length === 0) {
      console.log('No product IDs to fetch related products');
      return;
    }
    ...
    try {
      ...
      // Create separate arrays for each cart product's related products
      const productsWithRelatedProducts = data?.site?.products?.edges?.map((edge: any, index: number) => {
        const cartProduct = edge.node;
        const relatedProducts = cartProduct?.relatedProducts?.edges?.map((relatedEdge: any) => relatedEdge.node) || [];
        
        return {
          cartProductId: cartProductIds[index] || 'unknown',
          relatedProducts: relatedProducts
        };
      }) || [];
      
      console.log('Products with separate related arrays:', productsWithRelatedProducts);
      
      // Log each cart product and its related products separately
      productsWithRelatedProducts.forEach((product) => {
        console.log(`Cart Product ${product.cartProductId} Related Products:`, product.relatedProducts);
      });

      // Also log the combined array as before
      const allRelatedProducts = productsWithRelatedProducts.flatMap(p => p.relatedProducts);
      console.log('All Related Products Combined:', allRelatedProducts);
  ...
  return (
    ...
    >
      ...
      {optimisticLineItems.length > 0 && (
        <div className="mt-6">
          <button
            onClick={() => {
              const cartId = optimisticLineItems.map(item => item.id).join(', ');
              console.log(`Cart ID: ${cartId}`);
              // Here you would typically call an empty cart action
              optimisticLineItems.forEach(item => {
